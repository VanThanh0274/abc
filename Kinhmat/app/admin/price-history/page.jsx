...
export default function PriceHistoryPage() {
  ...
  const handleSave = async () => {
    if (!form.masp || !form.gianhap || !form.giaban) return toast.warn('Vui lòng điền đầy đủ thông tin');
    setSaving(true);
    try {
      await updatePrice({ masp: +form.masp, gianhap: +form.gianhap, giaban: +form.giaban });
          ...
          <motion.div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4"
            ...
            <motion.div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-7"
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-xs font-bold text-gray-600 uppercase mb-1 block">Chọn sản phẩm</label>
                  <select value={form.masp} onChange={e => setForm(f => ({ ...f, masp: e.target.value }))}
                ...
                <div>
                  <label className="text-xs font-bold text-gray-600 uppercase mb-1 block">Giá nhập (VNĐ)</label>
                  <input type="number" value={form.gianhap} onChange={e => setForm(f => ({ ...f, gianhap: e.target.value }))}
                    placeholder="VD: 500000" className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:ring-2 focus:ring-brand-gold/30 focus:border-brand-gold outline-none" />
                </div>
                <div>
